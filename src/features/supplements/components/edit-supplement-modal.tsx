// src/features/supplements/components/edit-supplement-modal.tsx
"use client"
...
export function EditSupplementModal({
  ...
  // Cargar datos del supplement cuando se abre el modal
  useEffect(() => {
    if (open && supplement) {
      // Cargar brands
      loadBrands()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: "Error al actualizar el suplemento. Intenta de nuevo." })
    } finally {
      setIsSubmitting(false)
    }
  }

  // Obtener la URL de imagen a mostrar (nueva preview, existente, o null)
              ...
              {displayImageUrl ? (
                <div className="relative group">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    {existingImageUrl ? "Haz clic en la X para reemplazar o remover la imagen" : "Haz clic en la X para remover la imagen"}
                  </p>
                </div>
              ) : (
